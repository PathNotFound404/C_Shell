...
- Your `hfsh26` shell program must be written in C/C++.  The use of the standard template library is permitted.
...
- Your source code file is `hfsh26.cpp`.
- The name of your executable must be `hfsh26`.
...
## Basic Shell: `hfsh26`
Your basic shell, called `hfsh26` (short for Horned Frog Shell v2), is basically an interactive loop: it repeatedly prints a prompt `hfsh26> ` (note the space after the greater-than sign), parses the input, executes the command specified on that line of input, and waits for the command to finish.  This is repeated until the user types `exit`.  

### The name of your final executable must be `hfsh26`.
...
At this point, `hfsh26` is running, and ready to accept commands. Type away!
...
```sh
prompt> ./hfsh26 batch.txt
```

One difference between batch and interactive modes: in interactive mode, a prompt is printed (`hfsh26> `).  In batch mode, no prompt should be printed.
...
The reading and parsing of lines of text has been completed for you!  See the file `hfsh26.cpp`.
...
# Points
 ...
 - Paths (Example test cases: 6, 7) (15 points)
